...
**Throttling:** if ESPN throttles (429) or returns a transient 5xx, `espnGet()` retries automatically with exponential backoff (up to 5 retries, honoring ESPN's `Retry-After` header when present) - check CloudWatch Logs for `console.warn` retry lines if a run takes longer than expected. The function's timeout is 10 minutes to give backoff room to work. It's meant to also carry `reserved_concurrent_executions = 1` (so invoking it again while a sync is already running fails fast instead of running concurrently), but that's temporarily omitted - this account's Lambda concurrency quota is stuck at AWS's 10-execution floor, which blocks reserving any capacity (see the comment in `modules/player-sync/main.tf`). Until a pending quota increase clears and that's added back, don't manually invoke this function while a previous run may still be in progress - check CloudWatch Logs first.
